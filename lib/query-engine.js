module.exports = function(root, requireModule) {
  ...
  function groupCondition(cb, flags) {
    var group = new this.constructor(this.options);
  ...
  class QueryEngine {
    constructor(_opts) {
      ...
      if (modelType) {
        modelType.iterateFields((field, fieldName) => {
          if (QueryEngine.prototype.hasOwnProperty(fieldName) || ('' + fieldName).match(/^(first|last|all|field)/))
    ...
    getConnector(modelType) {
      ...
      if (!connector)
        connector = this.getApplication().getConnectorFromQuery(this, modelType);
    ...
    getModelType(_opts) {
      ...
      if (!modelType)
        modelType = this.getApplication().getModelTypesFromQuery(this, _opts);
