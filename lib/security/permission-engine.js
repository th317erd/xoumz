module.exports = function(root, requireModule) {
  ...
  const PermissionEngine = this.defineClass((EngineBase) => {
    return class PermissionEngine extends EngineBase {
      static name() {
        return 'permission';
      }
      ...
      constructor(_opts) {
        ...
        if (opts.ownerRole !== false) {
          this.registerRole('owner', (opts.ownerRole instanceof Function) ? opts.ownerRole : (primary, target) => {
            ...
            var flags = primary.getRole(target);
            if (!flags)
              flags = 0;
      ...
      getContext(_opts) {
        return new Context(Object.assign({ name: 'permission', group: 'engine' }, _opts || {}));
      }
