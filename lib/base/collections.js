module.exports = function(root, requireModule) {
  ...
  class LazyItemFetcher {
    constructor(loaderCallback) {
      definePropertyRW(this, 'loaderCallback', loaderCallback);
    }

    async fetch() {
      return await this.loaderCallback();
    }
  }
  ...
  class LazyItem {
    constructor(item) {
      ...
      if (!(item instanceof LazyItemFetcher))
        definePropertyRO(this, '_result', item);
      else
        definePropertyRO(this, '_fetcher', item);
    }
    ...
    async fetch() {
      ...
      var loading = this._loading = this._fetcher.fetch(),
