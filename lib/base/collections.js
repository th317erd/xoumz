module.exports = function(root, requireModule) {
  ...
  function flattenItems(...args) {
    ...
    for (var i = 0, il = args.length; i < il; i++) {
      ...
      if (instanceOf(item, LazyCollection)) {
  ...
  const { definePropertyRO, definePropertyRW, equal, instanceOf } = requireModule('./base/utils');

  const LazyItem = this.defineClass((ParentClass) => {
    return class LazyItem extends ParentClass {
      constructor(item) {
  ...
  const LazyCollection = this.defineClass((ParentClass) => {
    return class LazyCollection extends ParentClass {
      ...
      constructor(...incomingItems) {
        ...
        definePropertyRW(this, 'length', undefined, () => this._items.length, (set) => {
          if (!set)
            return;
      ...
      async reduce(callback, _initial) {
        ...
        if (arguments.length === 1 && !this._items.length)
      ...
      async reduceRight(callback, _initial) {
        var initial = _initial,
            firstIndex = this._items.length - 1;
