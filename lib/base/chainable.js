  ...
  const ChainableConsumer = this.defineClass((ParentClass) => {
    return class ChainableConsumer extends ParentClass {
      ...
      getContextName(opts) {
        ...
        if (opts && typeof opts.getContextName === 'function')
          return opts.getContextName.call(opts);

        return (opts) ? opts.context : opts;
      }

      getProp(propName, _opts) {
        var opts = _opts || {},
            contextName = this.getContextName(opts) || '*',
            specifiedContext = this._contexts[contextName],
            propValue = (!specifiedContext || !specifiedContext.hasOwnProperty(propName)) ? this._contexts['*'][propName] : specifiedContext[propName];

        if ((opts.unwind || propName === 'value') && typeof propValue === 'function')
