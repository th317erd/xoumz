  ...
  const QueryBuilder = this.defineClass((ParentClass) => {
    return class QueryBuilder extends ParentClass {
      constructor(_opts) {
        ...
        var modelClass = opts.modelClass;
        if (modelClass) {
        ...
        definePropertyRO(this, 'type', (resourceName) => {
          this._currentOpDirty = (this._currentOp.type !== resourceName);
          this._currentOp.type = resourceName;
      ...
      getConnector(modelClass) {
        ...
        if (!(connector instanceof ConnectorBaseClass))
          connector = this.getApplication().getConnector({ query: this, modelClass, operation: 'query' });
      ...
      getModelName(_opts) {
        var modelClass = this._options.modelClass;
        if (!modelClass) {
          modelClass = this.getApplication().getModelNamesFromQuery(this, Object.assign({
            query: this,
            operation: 'query'
          }, _opts || {}));
        }
        ...
        return (modelClass instanceof Array) ? modelClass : [modelClass];
      }

      async executeQuery() {
        var modelClasss = this.getModelName(),
            // If there are multiple model types the order is not guaranteed
            promises = modelClasss.map((modelClass) => {
              return this.getConnector(modelClass).query(this, { ...this._options, modelClass });
            });
