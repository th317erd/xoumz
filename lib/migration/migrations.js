...
module.exports = function(root, requireModule) {
  const { noe, instanceOf } = requireModule('./base/utils');
  ...
  class MigrationEngine {
    ...
    async executeMigration(_migration) {
      ...
      if (noe(migration)) {
        Logger.error(`No migration found: ${_migration}`);
        return;
      }
    ...
    async executeMigrations(migrations) {
      ...
      try {
        ...
        await application.writeToStorage('lastMigrationRunTime', lastRunTime + 1);
      }
    }
  }
