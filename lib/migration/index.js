...
module.exports = function(root, requireModule) {
  ...
  class MigrationEngine {
    ...
    filterMigrations(migrations, _lastMigration) {
      ...
      var lastMigration = _lastMigration || 0;
      return migrations.filter((mo) => (mo.ts > lastMigration));
    }

    async getPendingMigrations() {
      var migrations = await this.getMigrations(),
          lastMigration,
          pendingMigrations = this.filterMigrations(migrations, lastMigration);
    ...
    async executeMigrations(migrations) {
      ...
      var application = this.getApplication(),
          migrationBaseType = application.wrapClass(this.getBaseMigrationType());
