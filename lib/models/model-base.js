module.exports = function(root, requireModule) {
  function defineModelField(fieldName, field, valueCache) {
    ...
    function convertPrimitiveValue(rawValue, helperFuncName = 'getter', raw = false) {
      function convertValue() {
        var primitiveType = (typeof ModelClass.primitive === 'function') ? ModelClass.primitive() : null;

        if (raw && !instanceOf(value, ModelClass))
          return (value == null) ? new ModelClass() : new ModelClass(value.valueOf());

        if (helperFuncName === 'getter' && value != null && primitiveType)
          return value.valueOf();

        if (value != null && !instanceOf(value, ModelClass))
          value = new ModelClass(value.valueOf());

        return value;
      }

      var helperFunc = field.getProp(helperFuncName),
          value = (typeof helperFunc === 'function') ? helperFunc.call(this, rawValue, { field, fieldName }) : rawValue;
    ...
    Object.defineProperty(this, `getRawField_${fieldName}`, {
      ...
      enumerable: false,
      configurable: true,
      value: () => convertPrimitiveValue.call(this, defaultValue, undefined, true)
    });
  }
  ...
  const ModelBase = generateModelMethods(this.defineClass((Permissible) => {
    return class ModelBase extends Permissible {
      constructor(_decomposedModel, ...args) {
        ...
        definePropertyRO(this, '_fieldDefinition', this.getFieldDefinition());
