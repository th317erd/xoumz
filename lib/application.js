...
import Utils, { definePropertyRW, definePropertyRO, sizeOf, noe, instanceOf, prettify } from './utils';
  ...
  class Application {
    ...
    async saveModels(models, _opts) {
      ...
      for (var i = 0, il = connectors.length; i < il; i++) {
        ...
        promises.push(schemaEngine.saveModels(connector, models, opts));
      }

      return Promise.all(promises);
    }

    async loadModels(params, _opts) {
      var schemaEngine = this.getSchemaEngine(),
          opts = _opts || {},
      ...
      return schemaEngine.loadModels(connector, params, opts);
    }
  }
