...
(function(root) {
  ...
  class Application {
    constructor(_opts) {
      ...
      this.registerEngine(null, this.constructor, { configKey: configPrefix });
      this.registerEngine('schemaEngine', this.SchemaEngine, { configKey: `schema` });
      this.registerEngine('connectorEngine', this.ConnectorEngine, { configKey: `connectors` });
      this.registerEngine('migrationEngine', this.MigrationEngine, { configKey: `migration` });
      this.registerEngine('permissionEngine', this.PermissionEngine, { configKey: `permissions` });
      this.registerEngine('httpServer', this.HTTPServer, { altName: 'HTTPServer', configKey: `http` });
      this.registerEngine('routeEngine', this.RouteEngine, {
        configKey: `${configPrefix}.routes`,
        initCallback: this.engineDependsOn('httpServer'),
        startCallback: this.engineDependsOn('httpServer'),
        finalizeCallback: this.engineDependsOn('httpServer')
      });

      if (sizeOf(opts.plugins)) {
        for (var i = 0, il = opts.plugins.length; i < il; i++) {
          var plugin = opts.plugins[i],
              thisEngine = this.requireModule(plugin);

          Object.assign(this, thisEngine);
        }
      }
    }
    ...
    getEngine(name) {
      ...
      var WrappedEngineClass = this.wrapClass(thisEngine.moduleClass);
    ...
    registerEngine(_name, moduleClass, _opts) {
      var prettyName = (_name) ? this.Utils.capitalize(_name) : '',
      ...
      if (!proto.hasOwnProperty(eventName))
        proto[eventName] = async function() {};

      if (!proto.hasOwnProperty(beforeEventName))
        proto[beforeEventName] = async function() {};

      if (!proto.hasOwnProperty(afterEventName))
    ...
    // getSchemaEngine() {
    //   return this.getEngine('schemaEngine', this.options.schema, this.SchemaEngine);
    // }

    // getConnectorEngine() {
    //   return this.getEngine('connectorEngine', this.options.connectors, this.ConnectorEngine);
    // }

    // getMigrationEngine() {
    //   return this.getEngine('migrationEngine', this.options.migration, this.MigrationEngine);
    // }

    // getPermissionEngine() {
    //   return this.getEngine('permissionsEngine', this.options.permissions, this.PermissionEngine);
    // }

    // getHTTPServer() {
    //   return this.getEngine('httpServer', this.options.http, this.HTTPServer);
    // }

    // getRouteEngine() {
    //   return this.getEngine('routeEngine', this.options.routes, this.RouteEngine);
    // }
