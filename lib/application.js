  ...
  class Application {
    ...
    getVersion() {
      ...
      if (!version) {
        ...
        throw new Error('Application version must be defined in a non-development environment');
      }
    ...
    wrapClass(Klass) {
      ...
      if (!('getConfigValue' in proto)) {
        Object.defineProperty(proto, 'getConfigValue', {
          writable: true,
          enumerable: false,
          configurable: true,
          value: (key, defaultValue) => {
            ...
            return configEngine.getConfigValue(key, defaultValue);
          }
        });
      }

      if (!('setConfigValue' in proto)) {
        Object.defineProperty(proto, 'setConfigValue', {
          writable: true,
          enumerable: false,
          configurable: true,
          value: (key, data) => {
            var configEngine = this.getEngine('config');
            if (!configEngine)
              throw new Error(`No defined config engine to save value: ${key}`);

            return configEngine.setConfigValue(key, data);
          }
        });
      }

      if (!('getEnvironment' in proto)) {
        Object.defineProperty(proto, 'getEnvironment', {
          writable: true,
          enumerable: false,
          configurable: true,
          value: () => {
            ...
            return configEngine.getEnvironment();
          }
        });
      }
