...
(function(root) {
  class Application {
    constructor(_opts) {
      ...
      const PermissionEngine = this.requireModule('./permission-engine');

      Object.assign(this, Schema, Connectors, Migration, QueryEngine, PermissionEngine, {
        Logger,
        Utils,
        Models
      });
    ...
    async start() {
      try {
        ...
        await this.onInit(schemaEngine, connectorEngine);
        await schemaEngine.start();
        await this.onAfterInit(schemaEngine, connectorEngine);
    ...
    getPermissionEngine() {
      ...
      var PermissionEngineClass = this.wrapClass(this.PermissionEngine);
      this._permissionEngine = new PermissionEngineClass(this.options.permissions);
