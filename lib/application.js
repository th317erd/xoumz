(function(root) {
  ...
  class Application {
    ...
    async registerEngine(name, engine) {
      this._engines[name] = {
        name,
        engine,
      ...
      if (this._state & APP_STATES.INITIALIZED && !(this._state & APP_STATES.STOPPED))
        await engine.initialize();

      if (this._state & APP_STATES.STARTED && !(this._state & APP_STATES.STOPPED))
        await engine.start();
    }
    ...
    async registerEngines(engines) {
      ...
      for (var [ name, engine ] of engines.entries())
        promises.push(this.registerEngine(name, engine));
    ...
    async initialize() {
      const defaultEngines = {
        ...
        [this.PermissionEngine.name()]: () => {
          return new this.PermissionEngine(this._options);
        },
        [this.SchemaEngine.name()]: () => {
          return new this.SchemaEngine({
      ...
      for (var [ name, engineCreator ] of defaultEngines.entries()) {
        if (!this._engines[name])
          promises.push(this.registerEngine(name, engineCreator()));
      }
