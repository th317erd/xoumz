module.exports = function(root, requireModule) {
  ...
  class SchemaValidationReport {
    ...
    generateMigration() {
      return `
      this.queue(this.getConnector('${this.connector.getContext()}').migrate(this.getSchemaEngine(), {}));
      `;
    }
    ...
    static fromDatabaseSchema(connector, schemaEngine, databaseSchemaEngine) {
      var report = new SchemaValidationReport({
        connector,
        schemaEngine
      });

      schemaEngine.compareTo(databaseSchemaEngine, report.addToReport.bind(report));
