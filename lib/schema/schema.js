module.exports = function(root, requireModule) {
  ...
  class SchemaEngine {
    ...
    getSchemaTypes() {
      ...
      SchemaTypes.iteratePrimitiveSchemaTypes((name, typeClass) => {
        definePropertyRO(cachedSchemaTypes, name, undefined, () => {
          ...
          if (typeClass.requiresArguments)
            return (...args) => { return new typeClass(this, ...args); };
      ...
      for (var i = 0, il = keys.length; i < il; i++) {
        ...
        (function(typeName, typeClass) {
          definePropertyRO(cachedSchemaTypes, typeName, undefined, () => {
            return new typeClass(this);
          }, () => {
            throw new Error('You can not attempt to assign a value to a schema type');
          });
    ...
    async start() {
      ...
      var schemaTypes = this.getSchemaTypes();
      ...
      this.iterateModelSchemas((typeInfo, typeName) => {
        ...
        typeInfo.modelType.iterateFields((field, fieldName) => {
          var fieldTypeName = field.getTypeName(),
              targetTypeNames = field.getTargetTypeName(),
          ...
          for (var i = 0, il = targetTypeNames.length; i < il; i++) {
            var targetTypeName = targetTypeNames[i],
                modelType = this.getModelSchema(targetTypeName);
            ...
            if (!modelType.hasField('ownerID')) {
              modelType.addField(schemaTypes.String.field('ownerID'));
            }
            
            if (!modelType.hasField('ownerType')) {
              modelType.addField(schemaTypes.String.field('ownerType'));
            }

            if (!modelType.hasField('ownerField')) {
              modelType.addField(schemaTypes.String.field('ownerField'));
            }
          }
        });

        this.getModelSchema(typeName).iterateFields((field, fieldName) => {
          console.log(`${typeInfo.modelType.getTypeName()} field: ${fieldName}`);
        });
      }, true);
    ...
    getTypeParentClass(typeName) {
      ...
      if (!parentType)
        return this.getApplication().wrapClass(this.getBaseRecordClass());
