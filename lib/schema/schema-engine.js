module.exports = function(root, requireModule) {
  const { definePropertyRO, definePropertyRW, instanceOf, noe, isCyclic } = requireModule('./base/utils');
  ...
  class SchemaEngine {
    ...
    constructor(_opts) {
      var opts = Object.assign({
        registerPrimitiveSchemaTypes: true
      }, _opts || {});
    ...
    getModelBaseClass() {
      return this.getApplication().getModelBaseClass();
    }

    getModelTypeClass() {
      return this.getApplication().getModelTypeClass();
    }

    getSchemaTypeClass() {
      return this.getApplication().getSchemaTypeClass();
    }
          ...
          TypeKlass = class GenericSchemaType extends SchemaTypeClass {
            ...
            async decompose(_val, _opts) {
              ...
              var opts = _opts || {},
                  ...
                  rawVal = {},
                  subVals = [new DecomposedModel({ modelType: registrationScope.modelType, model: inputValue, value: rawVal })];
              ...
              modelType.iterateFields((field, fieldName, index, flags) => {
                ...
                if (!(flags & ModelType.FLAGS.PRIMITIVE) || flags & ModelType.FLAGS.COMPLEX) {
              ...
              return DecomposedModelCollection.from(await Promise.all(subVals));
            }
            ...
            validate(_val, _opts) {
              ...
              if (modelTypeClass && modelTypeClass.validate instanceof Function)
                return modelTypeClass.validate.call(this, _val, _opts);
              ...
              var thisOpts = Object.assign({ op: 'save' }, opts, { owner: inputValue });
              var errors = modelType.iterateFields((field, fieldName) => {
                return field.validate((inputValue) ? inputValue[fieldName] : undefined, thisOpts);
              });

              return errors.reduce((arr, ret) => {
                if (noe(ret))
                  return arr;
            ...
            instantiate(data, _opts, ...args) {
              if (registrationScope.modelTypeClass.instantiate instanceof Function)
                return registrationScope.modelTypeClass.instantiate(data, _opts, ...args);

              if (data instanceof ModelBase)
                return data;

              if (data instanceof DecomposedModel)
                data = data.getValue();
              ...
              var opts = _opts || {},
                  model = new registrationScope.modelTypeClass(data, _opts, ...args);

              if (opts.onModelCreate instanceof Function)
                return opts.onModelCreate.call(model, data, opts);
