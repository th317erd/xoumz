module.exports = function(root, requireModule) {
  ...
  const SchemaEngine = this.defineClass((ParentClass) => {
    return class SchemaEngine extends ParentClass {
      static name() {
        return 'schema';
      }
      ...
      async start() {
        function defineModel(_typeName, model) {
          ...
          if (!(modelClass.getType instanceof Function)) {
            ...
            const GenericSchemaType = class GenericSchemaType extends SchemaTypeClass {
              constructor() {
                super(modelClass);
              }
            }
