            ...
            decompose(_val, _opts) {
              ...
              modelType.iterateFields((field, fieldName, index, flags) => {
                ...
                if (!(flags & ModelType.FLAGS.PRIMITIVE) || complex) {
                  var decomposedValue = field.decompose(value, { ...opts, owner: (complex) ? (opts.owner || inputValue) : value, ownerField: field, ownerType: modelType });
                  if (fieldName === 'stuff')
                    console.log('Decomposed value: ', field.getTypeName(), decomposedValue, value);
