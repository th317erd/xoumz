module.exports = function(root, requireModule) {
  function generateModelMethods(klass, _typeName) {
    ...
    if (!klass.hasOwnProperty('getSchemaType')) {
      klass['getSchemaType'] = function getSchemaType(_opts) {
        const opts = _opts || {},
              SchemaTypeClass = (typeof klass.getSchemaTypeClass === 'function') ? klass.getSchemaTypeClass(opts) : SchemaType;
    ...
    if (!klass.hasOwnProperty('getResourceName')) {
      klass['getResourceName'] = function getResourceName() {
    ...
    if (!klass.hasOwnProperty('rebindStaticMethod')) {
      klass['rebindStaticMethod'] = function rebindStaticMethod(methodName, method, klass) {
        // These will only be defined if the static method in question isn't defined on the model
        ...
        if (methodName === 'getModelName') {
          ...
          return function getSchemaType(_opts) {
            const opts = _opts || {},
                  SchemaTypeClass = (typeof klass.getSchemaTypeClass === 'function') ? klass.getSchemaTypeClass(opts) : SchemaType;
            return new SchemaTypeClass(klass, new Context({ name: 'type', group: 'schema' }, opts));
          };
        ...
        } else if (methodName === 'getResourceName') {
          return function getResourceName() {
    ...
    if (!klass.prototype.hasOwnProperty('getResourceName')) {
      klass.prototype['getResourceName'] = function getResourceName(...args) {
        return this.constructor.getResourceName(...args);
      };
    }
    ...
    if (!klass.prototype.hasOwnProperty('decompose')) {
      // WIP: decomposing models
      klass.prototype['decompose'] = async function decompose(field, fieldName) {
        async function buildFieldDecomposePromise(field, fieldName) {
          var rawField = this.getRawField(fieldName),
              value = await rawField.decompose(field, fieldName, this);

          return { model: rawField, field, fieldName, value };
        }
        ...
        for (var [ fieldName, field ] of schema.entries({ hidden: false, virtual: false }))
          promises.push(buildFieldDecomposePromise.call(this, field, fieldName));

        var values = await Promise.all(promises);
        for (var i = 0, il = values.length; i < il; i++) {
          var data = values[i];
          fieldData[data.fieldName] = new DecomposedModel(data.value, { model: data.model, field: data.field, fieldName: data.fieldName });
        }

        return new DecomposedModel(fieldData, { model: this, field, fieldName });
      };
    }
  ...
  const ModelSchema = this.defineClass((ParentClass) => {
    return class ModelSchema extends ParentClass {
      ...
      getResourceName() {
        return this.getModelClass().getResourceName();
      }
      ...
      getVersioned(_opts) {
        return this.getApplication().getModelSchema(this.getResourceName(), _opts);
      }
