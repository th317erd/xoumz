module.exports = function(root, requireModule) {
  ...
  function generateModelMethods(klass, _typeName) {
    var typeName = _typeName || klass.name,
        modelSchema;
    ...
    if (!klass.hasOwnProperty('getTypeName')) {
      klass['getTypeName'] = function getTypeName() {
        return typeName;
      };
    }
      ...
      klass['rebindStaticMethod'] = function rebindStaticMethod(methodName, method, klass) {
        if (methodName === 'getTypeName') {
          ...
          return function getUniqueResourceID() {
            return `org/xoumz/model/${klass.getTypeName()}`;
          };
    ...
    if (!klass.prototype.hasOwnProperty('getTypeName')) {
      klass.prototype['getTypeName'] = function getTypeName(...args) {
        return this.constructor.getTypeName(...args);
      };
    }
  ...
  const Schema = this.defineClass((ParentClass) => {
    return class Schema extends ParentClass {
      ...
      getContext(...args) {
        return new Context({ name: 'schema', group: 'schema' }, ...args, { modelClass: this.getModelClass() });
      }
  ...
  const ModelSchema = this.defineClass((ParentClass) => {
    return class ModelSchema extends ParentClass {
      ...
      getRawSchemaDefinition(types, typeName, rawSchema) {
        if (!rawSchema)
        ...
        var defaultFields = this.getDefaultFields(this.primitive()),
            finalSchema = new Schema(this._model);

        // Check that all field names are set
        this.iterateRawSchema(typeName, rawSchema, (_field, fieldName) => {
          var field = _field;
      ...
      defineSchema(types, _parent, definition) {
        ...
        definition.rawSchema = this.getRawSchemaDefinition.call(this, types, typeName, definition.schema.call(this, types, typeName, (parentDefinition) ? Object.assign({}, parentDefinition.rawSchema) : null, parentDefinition));
      ...
      initialize() {
        ...
        schemaFactory.call(this, this.defineSchema.bind(this, schemaEngine.getTypes()));
      }

      cloneWithVersion(_version) {
        var version = getVersion.call(this, null, _version),
            copy = new this.constructor(this._model, Object.assign({}, this._options, { skipInitialize: true }));
      ...
      getVersionList() {
        return Object.keys(this._schemaDefinitions);
      }
      ...
      *[Symbol.iterator]() {
      ...
      getField(fieldName, opts) {
        var version = getVersion.call(this, opts),
            rawSchema = this._schemaDefinitions[version].rawSchema;
      ...
      getFieldProp(fieldName, propName, opts) {
        var field = this.getField(fieldName, opts);
        return (field) ? field.getProp(propName, opts) : undefined;
      }
    };
  }, undefined, {
    // Static properties
