module.exports = function(root, requireModule) {
  ...
  class ModelSchema {
    ...
    getRawSchema(types, typeName, rawSchema) {
      ...
      if (!finalSchema.id)
        finalSchema.id = this.createDefaultPrimaryKeyField(types, typeName);

      if (!finalSchema.createdAt)
        finalSchema.createdAt = this.createDefaultCreatedAtField(types, typeName);

      if (!finalSchema.updatedAt)
        finalSchema.updatedAt = this.createDefaultUpdatedAtField(types, typeName);
    ...
    entries(_opts, _version) {
      ...
      return this._schemaDefinitions[version].rawSchema.entries(_opts);
    }

    keys(_opts, _version) {
      var version = getVersion.call(this, _opts, _version);
      return this._schemaDefinitions[version].rawSchema.keys(_opts);
    }

    values(_opts, _version) {
      var version = getVersion.call(this, _opts, _version);
      return this._schemaDefinitions[version].rawSchema.values(_opts);
    }
