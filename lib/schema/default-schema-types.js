  ...
  function getPrimitiveSchemaTypes(SchemaType, ModelType) {
    ...
    function parseArrayValue(_val, _opts) {
      ...
      var opts = _opts || {},
          ...
          orderField = modelType.getOwnerOrderField();

      // TODO: Handle LazyItems and DecomposedModels
      if (orderField && (_val instanceof Array)) {
        var orderFieldName = modelType.getFieldProp(orderField, 'field');
        if (!noe(orderFieldName)) {
          val = val.map((item) => {
            return ((item && item.getRawValue instanceof Function)) ? item.getRawValue() : item;
          }).sort((a, b) => {
            var x = a[orderFieldName],
                y = b[orderFieldName];
      ...
      for (var i = 0, il = val.length; i < il; i++) {
        ...
        if (typeInfo.primitiveType) {
          finalVal.push((new typeInfo.primitiveType(schemaEngine, modelType)).instantiate(thisVal, opts));
        } else {
          var model = modelType.instantiate(thisVal, opts);
          finalVal.push(model);
        }
      }
