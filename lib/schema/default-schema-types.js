module.exports = function(root, requireModule) {
  ...
  //   async function decomposePrimitive(_val, asPrimitive, _opts) {
  //     var val = coerceValue(_val),
  //         opts = _opts || {},
  //         modelType = this.getModelType(),
  //         getter = this.getProp('getter', opts.context),
  //         value = getter(this, val);

  //     return (asPrimitive) ? value : await modelType.instantiate({ value: value }, opts).decompose(opts);
  //   }
  ...
  //       this.getter(function(field, val) {
  //         return moment(val, field.getProp('format')).utc().toISOString();
  //       });
  ...
  //       val.map((item, index) => {
  //         var decomposedValue = internalType.decomposeAsModel(item, {
  //           ...opts,
  //           _order: index
  //         });
  ...
  //       return DecomposedModelCollection.from(await Promise.all(parts));
  //     }
