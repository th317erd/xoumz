module.exports = function(root, requireModule) {
  ...
  function getPrimitiveSchemaTypes(SchemaType, ModelType) {
    function coerceValue(val) {
      return (val instanceof ModelType) ? val.value : val;
    }
    ...
    function parseStringValue(_val) {
      ...
      if (val && typeof val !== 'string' && !(val instanceof String)) {
        debugger;
        console.log('FUNKY STRING VALUE!', val);
      }
    ...
    function decomposePrimitive(_val, asPrimitive, _opts) {
      ...
      if (!asPrimitive)
        console.log('NOT AS PRIMITIVE: ', val);
    ...
    class ArrayOfType extends SchemaType {
      ...
      decompose(_val, _opts) {
        ...
        for (var i = 0, il = val.length; i < il; i++) {
          ...
          console.log('DECOMPOSED: ', val[i], decomposedValue);
