module.exports = function(root, requireModule) {
  ...
  class SchemaType {
    constructor(schemaEngine, parentModelType, typeName) {
      ...
      this.defineProp('setter', (val) => val, undefined, (setterFunc, name) => {
        defaultSchemaTypes.assertSchemaTypes('function')(setterFunc, name);
        var lastSetter = this.getProp('setter');

        return (_val, ...args) => {
          var val = _val;

          if (lastSetter)
            val = lastSetter.apply(this, val, ...args);

          return setterFunc.call(this, val, ...args);
        };
      });

      this.defineProp('getter', (val) => val, undefined, (getterFunc, name) => {
        defaultSchemaTypes.assertSchemaTypes('function')(getterFunc, name);
        var lastGetter = this.getProp('getter');

        return function(field, _val) {
          var val = _val;

          if (lastGetter)
            val = lastGetter.call(this, field, val);

          return getterFunc.call(this, field, val);
        };
      });
    }
    ...
    validator(cb, _opts) {
      ...
      var specifiedContext = getContext.call(this, this._context),
          validatorFunc = specifiedContext._validator;

      var validatorOpts = _opts || {};
      if (instanceOf(validatorOpts, 'string', 'number', 'boolean'))
        validatorOpts = { phase: [ ('' + validatorOpts) ] };

      definePropertyRW(specifiedContext, '_validator', (val, _opts) => {
        var opts = Object.assign({}, validatorOpts, _opts || {}),
            phase = opts.phase,
            ret;

        // If this is the wrong phase for this validator then abort
        if (phase && opts.op) {
          if (!(phase instanceof Array))
            phase = [phase];

          if (phase.indexOf(opts.op) < 0)
            return;
        }
        ...
        try {
          // If a previous validator was set, call it first
          if (validatorFunc instanceof Function) {
            ret = validatorFunc.call(this, val, opts);
            if (ret)
              errors = errors.concat(ret);
          }

          // Call validator
          ret = cb.call(this, val, opts);
          if (ret)
            errors = errors.concat(ret);
        } catch (e) {
          errors = errors.concat({ type: 'error', message: e.message, success: false });
        }
    ...
    validate(val, _opts) {
      ...
      var validator = this.getProp('validator', context);
      if (!(validator instanceof Function))
        return;

      return validator.call(this, val, opts);
    }
    ...
    async decompose(val, _opts) {
      throw new Error('Can not decompose a base SchemaType');
    }

    async decomposeAsModel(val, _opts) {
      throw new Error('Can not decompose a base SchemaType');
    }
