  ...
  const FieldDefinition = this.defineClass((ChainableConsumer) => {
    return class FieldDefinition extends ChainableConsumer {
      ...
      start(_modelClass, _opts, ...args) {
        ...
        const fieldDef = this._proxyTarget || this,
              FieldModelClass = app.defineClass((ModelClass) => {
                const modelName = ModelClass.getModelName(),
                      className = ModelClass.name || ModelClass.displayName;

                const Klass = (new Function('ModelClass', 'fieldDef', `
                  return class ${className} extends ModelClass {
                    static getModelName() {
                      return '${modelName}';
                    }
