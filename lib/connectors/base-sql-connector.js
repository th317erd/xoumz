    ...
    async readModelsFromIDs(schemaEngine, modelType, modelIDs, _opts) {
      ...
      var opts = _opts || {},
          ...
          primaryIDList = modelIDs.filter((id) => {
            ...
            return !modelMap.hasOwnProperty(thisID);
      ...
      try {
        ...
        decomposedModels = decomposedModels.filter((decomposedModel) => {
          var { modelType, typeName, value, modelID } = this.getDecomposedModelInfo(decomposedModel),
              thisID = `${typeName}:${modelID}`;

          if (!typeName)
            debugger;

          if (modelMap.hasOwnProperty(thisID))
            return false;
