module.exports = function(root, requireModule) {
  ...
  class BaseSQLConnector extends BaseConnector {
    ...
    stitchDecomposedModels(schemaEngine, decomposedModels, _opts) {
      ...
      for (var i = 0, il = decomposedModels.length; i < il; i++) {
        var decomposedModel = decomposedModels[i],
            { modelType, primaryKey, value, modelID } = this.getDecomposedModelInfo(decomposedModel);
        ...
        var parentModel = modelMap[value.ownerID],
            parentField = value.ownerField;
    ...
    async readPrimaryModels(schemaEngine, modelType, query, _opts) {
      ...
      try {
        ...
        var result = await this.exec(querySQL),
            decomposedModels = this.getRawModelDataFromRows(schemaEngine, result);
        ...
        var stichedModels = this.stitchDecomposedModels(schemaEngine, decomposedModels.concat(secondaryModels), opts);
        return stichedModels.map((decomposedModel) => {
    ...
    async readSecondaryModels(schemaEngine, modelType, primaryIDs, _opts) {
      ...
      var context = this.getContext(),
          ...
          }).filter((chunk) => !!chunk).join('\n')}\nWHERE ((${(hasSelfSubType) ? `${primaryTableName}.ownerType=${this.escape(primaryTypeName)} AND ${primaryTableName}.ownerID IN (${primaryIDList})) OR ` : ''}${primaryTableName}.id IN (${primaryIDList}))`;

      //console.log('Secondary Query: ', querySQL);
      try {
        var result = await this.exec(querySQL),
            decomposedModels = this.getRawModelDataFromRows(schemaEngine, result);

        decomposedModels = decomposedModels.filter((decomposedModel) => {
          var { value, modelID } = this.getDecomposedModelInfo(decomposedModel);
