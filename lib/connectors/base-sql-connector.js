    ...
    return class BaseSQLConnector extends BaseConnector {
      ...
      async buildTablesFromSchema(_opts) {
        ...
        for (var modelClass of schemaEngine.values()) {
          var resourceName = modelClass.getResourceName(),
              tableName = this.getTableNameFromModelName(modelClass, opts),
              hasTable = rawTableSchema.hasOwnProperty(tableName),
              modelIntgrity = integrity.getModelIntegrity(resourceName);

          // We have the table, and nothing has changed
          if (hasTable && !modelIntgrity)
            continue;
      ...
      modelClassToSQLType(field) {
        ...
        var resourceName = modelClass.getResourceName();
        if (resourceName === 'org_xoumz_model_Integer' || typeOf(modelClass, 'IntegerPrimitive')) {
          parts.push('INT');
        } else if (resourceName === 'org_xoumz_model_Decimal' || typeOf(modelClass, 'DecimalPrimitive')) {
          parts.push('DOUBLE');
        } else if (resourceName === 'org_xoumz_model_Boolean' || typeOf(modelClass, 'BooleanPrimitive', 'Boolean')) {
          parts.push('TINYINT(1)');
        } else if (resourceName === 'org_xoumz_model_String' || typeOf(modelClass, 'StringPrimitive', 'String')) {
          ...
          parts.push('VARCHAR');
          parts.push(`(${size})`);
        } else if (resourceName === 'org_xoumz_model_Date' || typeOf(modelClass, 'DatePrimitive', 'moment', 'Date')) {
          parts.push('DATETIME');
        }
