...
module.exports = function(root, requireModule) {
  ...
  const BaseSQLConnector = this.defineClass((BaseConnector) => {
    return class BaseSQLConnector extends BaseConnector {
      ...
      flattenQueries(_queries) {
        const doFlatten = (_queries) => {
          if (!_queries)
          ...
          var before = [],
              transactions = [],
              after = [],
              queries = ((_queries instanceof Array) ? _queries : [_queries]).map((query) => {
        ...
        var { before, transactions, after } = doFlatten(_queries),
            finalBefore = [],
            finalAfter = [];
        ...
        return [].concat(finalBefore, { query: 'BEGIN TRANSACTION', required: true }, transactions, { query: 'COMMIT', required: true }, finalAfter);
      }
      ...
      async buildTablesFromSchema(_opts) {
        ...
        await this.verifySchemaUpdate(schemaEngine, opts);
      ...
      async dropColumn(field, _opts) {
        ...
        var queries = await this.generateDropColumnQueries(modelClass, schemaField, tableName, columnName, opts);
      ...
      async addColumn(field, _opts) {
        ...
        var queries = await this.generateAddColumnQueries(modelClass, schemaField, tableName, columnName, opts);
        await this.execAll(queries, undefined, opts);
      }
