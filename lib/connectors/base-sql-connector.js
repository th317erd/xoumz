...
module.exports = function(root, requireModule) {
  const { noe, setProp, getProp } = requireModule('./base/utils');
    ...
    getModelTypeQueryInfo(modelType) {
      ...
      var ModelTypeClass = this.getApplication().getModelTypeClass(),
          ...
          tables = subTypes.concat({ typeName: primaryTypeName, modelType, tableName: primaryTableName }).map((type) => {
            ...
            type.fields = this.tableToNamedFields(schemaEngine, tableName, modelType);
    ...
    async destroyDecomposedModels(decomposedModels, _opts) {
      ...
      var promises = Object.keys(owners).map((ownerKey) => {
        var owner = owners[ownerKey],
            { modelType, typeName, primaryKeyValue, primaryKeyFieldName } = owner;
        ...
        var queries = tables.reduce((q, tableInfo) => {
          ...
          if (!noe(thisOwnerIDFieldName, thisOwnerTypeFieldName)) {
            q.push({
              query: `DELETE FROM ${tableName} WHERE (${thisOwnerIDFieldName}=${this.escape(primaryKeyValue)} AND ${thisOwnerTypeFieldName}=${this.escape(typeName)})`,
              required: true
            });
          }
        ...
        if (!opts.descendantsOnly) {
          ...
          queries.push({
            query: `DELETE FROM ${tableName} WHERE (${primaryKeyFieldName}=${this.escape(primaryKeyValue)})`,
            required: true
          });
        }
