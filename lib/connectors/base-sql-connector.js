module.exports = function(root, requireModule) {
  ...
  class BaseSQLConnector extends BaseConnector {
    ...
    queryEngineToQueryString(queryEngine) {
      function queryToConditional(_query, args) {
        // TODO: Complete query types / query engine

        var query = this.escape(_query);

        if (args.length === 1) {
          return `${query}=${this.escape(args[0])}`;
        } else {
          var q = [],
              min = args[0],
              max = args[1];
          ...
          if (!noe(min))
            q.push(`${query}>=${this.escape(min)}`);

          if (!noe(max))
            q.push(`${query}<=${this.escape(max)}`);
      ...
      var queryStr = [],
          flags = queryEngine.getFlags(),
          not = flags & QueryEngine.FLAGS.NOT,
          or = flags & QueryEngine.FLAGS.OR;

      queryEngine.iterateQueries((query, args) => {
        queryStr.push(queryToConditional.call(this, query, args));
      });

      var subGroup = queryEngine.iterateGroups(this.queryEngineToQueryString.bind(this));
      if (!noe(subGroup))
        queryStr.push(subGroup);

      return `(${queryStr.join((or) ? ' OR ' : ' AND ')})`;
    }

    async query(queryEngine, opts) {
      console.log('Query: ', this.queryEngineToQueryString(queryEngine));
