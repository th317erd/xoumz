module.exports = function(root, requireModule) {
  const { definePropertyRW, sizeOf, instanceOf } = requireModule('./utils');
  ...
  class ConnectorEngine {
    ...
    async validateConnectorSchema(schemaEngine, connector) {
      try {
        ...
        if (!(rawDatabaseSchema instanceof SchemaEngine))
          rawDatabaseSchema = await schemaEngine.schemaEngineFromRawSchema(rawDatabaseSchema);
    ...
    filterConnectors(connectors, _filter) {
  ...
  Object.assign(root, BaseConnector, MemoryConnector, SQLiteConnector, {
    QueryUtils,
    ConnectorEngine
  });
};
