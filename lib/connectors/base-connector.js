module.exports = function(root, requireModule) {
  ...
  const BaseConnector = this.defineClass((ParentClass) => {
    return class BaseConnector extends ParentClass {
      ...
      getContext(...args) {
        return new Context({ name: '_base', group: 'connector' }, ...args);
      }
      ...
      async getModelSchemaFromTableName(schemaEngine, tableName) {
        function getModelName() {
          for (var [ typeName, type ] of schemaEngine) {
            var modelSchema = type.getSchema(),
                versions = modelSchema.getVersionList();
      ...
      abstractTypeToFieldDefinition(schemaEngine, column, opts) {
        var schemaTypes = schemaEngine.getTypes(this.getContext(opts)),
      ...
      async databaseTableToModelType(schemaEngine, tableName, table) {
        var definedModelSchema = await this.getModelSchemaFromTableName(schemaEngine, tableName),
        ...
        var ModelClass = definedModelSchema.getBaseModelClass();
