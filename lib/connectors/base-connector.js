  ...
  const BaseConnector = this.defineClass((ParentClass) => {
    return class BaseConnector extends ParentClass {
            ...
            static schema(defineSchema) {
              return defineSchema(ModelClass.schema, {
                schema: function(types, modelName, parentSchema) {
                  return (parentSchema) ? this.merge(rawSchema) : rawSchema;
                },
      ...
      verifySchemaIntegrity(schemaEngine, tableModels) {
        ...
        for (var [ modelName, modelClass ] of tableModels) {
          ...
          var modelSchema = schemaModelClass.getSchema(),
              diff = this.areModelSchemasDifferent(modelSchema, modelClass.getSchema());
      ...
      async verifySchemaUpdate(schemaEngine, opts) {
        // WIP: Complete integrity check functions
        function isAddingFields(integrity) {
          return false;
        }

        function isDroppingFields() {
          return false;
        }

        function isCoercingFields() {
          return false;
        }
        ...
        var tables = await this.getSchema(),
            ...
            integrity = this.verifySchemaIntegrity(tableModels),
            environment = this.getEnvironment(),
            isDev = (environment === 'development'),
            autoMigrateOptions = this.getConfigValue('schema.autoMigrate', {
              allowCoerceColumn: isDev,
              allowDropColumn: isDev,
              allowAddColumn: isDev,
              allowDropTable: isDev,
              allowAddTable: isDev
            });
        ...
        for (var modelClass of schemaEngine.values()) {
          ...
          if (hasTable) {
            if (!integrity[modelName])
              continue;

            if (!autoMigrateOptions.allowDropTable && !tableModels.hasOwnProperty(modelName))
            ...
            if (!autoMigrateOptions.allowAddTable && tableModels.hasOwnProperty(modelName))
            ...
            var columnNames = isAddingFields(integrity);
            ...
            var columnNames = isDroppingFields(integrity);
            ...
            var columnNames = isCoercingFields(integrity);
