module.exports = function(root, requireModule) {
  ...
  const BaseConnector = this.defineClass((ParentClass) => {
    return class BaseConnector extends ParentClass {
      ...
      getContext(_opts) {
        return new Context(Object.assign({ name: '_base', group: 'connector' }, _opts || {}));
      }
      ...
      getTableNameFromModelType(modelType) {
        var tableName = modelType.getFieldProp('_table', 'value', this.getContext({ modelType }));
        if (noe(tableName))
          tableName = `${modelType.getTypeName()}_${modelType.getVersion()}`;
