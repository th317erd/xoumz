module.exports = function(root, requireModule) {
  ...
  const BaseConnector = this.defineClass((ParentClass) => {
    return class BaseConnector extends ParentClass {
            ...
            static schema(defineSchema) {
              return defineSchema(ModelClass.schema, {
                schema: function(types, modelName, parentSchema) {
                  return (parentSchema) ? parentSchema.merge(rawSchema) : rawSchema;
                },
      ...
      getModelNameInfoFromTypeName(schemaEngine, modelName) {
        var ModelNameClass = this.getApplication().getModelNameClass(),
            modelClass = schemaEngine.getModelName(modelName);

        if (!(modelClass instanceof ModelNameClass))
          throw new Error(`Connector (${this.context}) error: Can not read data: unknown or invalid schema type: ${modelName}`);

        return { modelClass, modelName, primaryKeyFieldName: this.getModelNamePrimaryKeyField(modelClass) };
      }
