module.exports = function(root, requireModule) {
  const { definePropertyRO, definePropertyRW, noe, instanceOf, sizeOf } = requireModule('./utils');
  ...
  class BaseConnector {
    ...
    getModelTypeInfoFromTypeName(schemaEngine, typeName) {
      ...
      return { modelType, typeName, primaryKey: this.getModelTypePrimaryKeyField(modelType) };
    }

    getDecomposedModelInfo(decomposedModel) {
      var ModelTypeClass = this.getApplication().getModelTypeClass(),
          { modelType, typeName, primaryKey, value, modelID } = decomposedModel;
      ...
      if (!primaryKey)
        primaryKey = decomposedModel.primaryKey = this.getModelTypePrimaryKeyField(modelType);
    ...
    mapDecomposedModels(decomposedModels) {
      var allOwners = {};
      ...
      for (var i = 0, il = decomposedModels.length; i < il; i++) {
        ...
        var { modelType, value } = decomposedModel;
        if (!modelType || !value)
          continue;

        var typeName = modelType.getTypeName(),
            primaryKeyValue = modelType.retrievePrimaryKeyValue(value);

        if (noe(typeName, primaryKeyValue))
          continue;

        var key = `${typeName}:${primaryKeyValue}`;
        allOwners[key] = decomposedModel;
      }
    ...
    getDecomposedModelOwners(decomposedModels) {
      ...
      for (var i = 0, il = decomposedModels.length; i < il; i++) {
        ...
        var modelType = application.getModelType(ownerType, { context, model: decomposedModel, operation: 'write' });
        if (!modelType)
          throw new Error(`Can not figure out model type for owner type ${ownerType}`);

        var primaryKeyFieldName = this.getModelTypePrimaryKeyField(modelType, { context: this.getContext() });
        owners[key] = {
          modelType,
          typeName: ownerType,
          primaryKeyValue: ownerID,
          primaryKeyFieldName
        };
      }
    ...
    async destroyDescendants(decomposedModels, _opts) {
      throw new Error(`Connector [${this.context}] doesn't implement "destroyDescendants" method`);
    }
    ...
    getDecomposedModelAndInfo(data, _opts) {
      ...
      var items = (data instanceof DecomposedModel) ? data : data.decompose();
