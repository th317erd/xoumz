module.exports = function(root, requireModule) {
  ...
  // WIP: Change all connectors to use unique resource id instead of model names
  ...
  const BaseConnector = this.defineClass((ParentClass) => {
    return class BaseConnector extends ParentClass {
      ...
      getTableNameFromModelName(modelClass, _opts) {
        ...
        if (noe(tableName))
          tableName = `${modelClass.getResourceName()}_${modelSchema.getVersion()}`;
          ...
          return class GenericModel extends ModelClass {
            ...
            static schema(defineSchema) {
              return defineSchema(ModelClass.schema, {
                schema: function(types, resourceName, parentSchema) {
      ...
      async getSchema(_opts) {
        ...
        for (var i = 0, il = tableNames.length; i < il; i++) {
          ...
          group[modelClass.getResourceName()] = modelClass;
        }
      ...
      async verifySchemaUpdate(schemaEngine, opts) {
        ...
        var schemaTypeNames = schemaEngine.getSchemaTypeNames({ real: true, resourceNames: true }),
        ...
        for (var i = 0, il = allKeys.length; i < il; i++) {
          var resourceName = allKeys[i],
              modelClass = schemaEngine.getModelClass(resourceName),
              modelName = (modelClass) ? modelClass.getModelName() : '???',
              hasTable = tableModels.hasOwnProperty(resourceName);

          if (hasTable) {
            if (!integrity.getModelIntegrity(resourceName))
              continue;
