...
module.exports = function(root, requireModule) {
  ...
  const SQLiteConnection = this.defineClass((ParentClass) => {
    return class SQLiteConnection extends ParentClass {
      ...
      getContext(_opts) {
        return new Context(Object.assign({ name: 'sqlite:connection', group: 'connection' }, _opts || {}));
      }
  ...
  const SQLiteConnector = this.defineClass((BaseSQLConnector) => {
    return class SQLiteConnector extends BaseSQLConnector {
      ...
      getContext(_opts) {
        return new Context(Object.assign({ name: 'sqlite', group: 'connector' }, _opts || {}));
      }
      ...
      generateTableCreateQuery(modelType, _opts) {
        var primitiveType = modelType.getModelClass().primitive();
        ...
        var opts = _opts || {},
            ...
            tableName = this.getTableNameFromModelType(modelType);

        query.push(`CREATE TABLE ${tableName} (`);
        var index = 0;

        for (var [ fieldName, field ] of modelType.entries({ hidden: false, virtual: false, primitive: true })) {
          if (index > 0)
            query.push(', ');
