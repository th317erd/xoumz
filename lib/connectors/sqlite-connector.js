...
module.exports = function(root, requireModule) {
  const { definePropertyRO, definePropertyRW, noe } = requireModule('xoumz://utils');
  ...
  class SQLiteConnector extends BaseSQLConnector {
    ...
    async writeRaw(decomposedModel, _opts) {
      var opts = _opts || {},
          modelType = decomposedModel.modelType,
      ...
      try {
        var querySQL = `SELECT ${primaryKeyFieldName} FROM ${tableName} WHERE ${primaryKeyFieldName}=${this.escape(primaryKeyValue)}`,
      ...
      if (updateOperation)
        var query = `UPDATE ${tableName} SET ${values.map((v, i) => `\`${columnNames[i]}\`=${v}`).join(',')} WHERE ${primaryKeyFieldName}=${this.escape(primaryKeyValue)}`;
      else
