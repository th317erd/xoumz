...
module.exports = function(root, requireModule) {
  ...
  class SQLiteConnector extends BaseSQLConnector {
    ...
    generateTableCreateQuery(schemaEngine, tableName, modelType, _opts) {
      var opts = _opts || {},
          query = [],
      ...
      }, { context, virtual: false, primitive: true, model: false });
    ...
    generateTableUpdateQueries(schemaEngine, modelType, tableName, _opts) {
      var opts = _opts || {},
          { create } = opts,
          context = this.getContext(),
          queries = [],
      ...
      var createQuery = this.generateTableCreateQuery(schemaEngine, tableName, modelType, opts);
    ...
    async writeRaw(schemaEngine, decomposedModel, _opts) {
      var modelType = decomposedModel.modelType,
          ...
          primaryKeyField = modelType.getFieldProp(modelType.getPrimaryKeyField(), 'field', context),
          primaryKeyValue = model[primaryKeyField];

      if (noe(primaryKeyValue)) {
        console.log('BAD!', primaryKeyField, primaryKeyValue, model);
