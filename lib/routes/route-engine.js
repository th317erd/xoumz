  ...
  const RouteEngine = this.defineClass((EngineBase) => {
    return class RouteEngine extends EngineBase {
      ...
      generateCRUDRoutesFromSchema(_prefix) {
        ...
        schemaEngine.iterateModelSchemas((typeInfo, modelName) => {
          if (typeInfo.primitiveType)
            return;

          var modelClass = typeInfo.modelClass;
          this.registerRoute((Route) => {
            var matchingRE = new RegExp(`^${regExpEscape(prefix)}/(${regExpEscape(modelName.toLowerCase())})(?:/(.*))?`, 'i');
