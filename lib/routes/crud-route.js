module.exports = function(root, requireModule) {
  ...
  const CRUDRoute = this.defineClass((Route) => {
    return class CRUDRoute extends Route {
      ...
      getModelClass() {
        throw new Error('CRUD route doesn\'t provide a "getModelName" method');
      }
      ...
      async onCreate({ id }, body) {
        ...
        try {
          var model = modelClass.instantiate(body || {}, { validate: true }),
              errors = [],
      ...
      async onRetrieve({ id }, body) {
        var modelClass = this.getModelClass();
        return this.renderResponse(`GET Hello from model handler: ${modelClass.getModelName()} = ${id}`);
      }
